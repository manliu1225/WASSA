...
from .blstm_senti import BiLSTMSentiClassifier
from .conNet import ConNetlassifier
from .word_sentifeature import Extract_wordsentifeature
...
class BiLSTMSentiFeatureClassifier(BaseSentimentClassifier):

  def __init__(self, **kwargs):
    super(BiLSTMSentiFeatureClassifier, self).__init__(classifier=BiLSTMSentiClassifier(), **kwargs)
  ...
  def _make_preprocessor(self):
    ...
    preprocessor = make_pipeline(
      BasicTokenizer(),
      make_union(
        # make_pipeline(CMUArkTweetPOSTagger(), ListCountVectorizer(lowercase=False, binary=True), Print()),  # POS features
...
class WordSentimentVector(BaseEstimator, TransformerMixin):
    def __init__(self, word2vec):
        self.word2vec = word2vec
        self.dim = len(word2vec.itervalues().next())
    ...
    def transform(self, X):
      ...
      for line in X:
        ...
        for i, word in enumerate(line):
          embeddingvector = self.word2vec[word] if word in self.word2vec else np.zeros(self.dim)
          wordsentpipeline = Extract_wordsentifeature()
          y = []
          sentivector = wordsentpipeline.pos_pipeline().fit_transform(line, y)
          linevector.append(np.append(embeddingvector , sentivector.toarray()[i]))
        X_new.append(linevector)
      X_new = np.array(X_new)
      X = pad_sequences(X_new, self.MAX_LENGTH)
#      print 'in MeanEmbeddingVectorizer X.shape is ...'
#     print X.shape
...
class WordVector(WordSentimentVector):
  def __init__(self, **kwargs):
    # W2V_FILE = os.path.join(os.getcwd(), "tweets/resources/tweet_glove/glove.twitter.27B.50d.txt")
    with open(W2V_FILE, "rb") as lines:
      w2v = {line.split()[0]: np.array(map(float, line.split()[1:])) for line in lines}
    super(WordVector, self).__init__(w2v)
