...
class Extract_wordsentifeature(object):
  def lexicon_pipeline(self,lexicon): return make_pipeline(LexiconFeatures(lexicon), DictVectorizer())
  def pos_pipeline(self): return  make_pipeline(CMUArkTweetPOSTagger(), ListCountVectorizer(lowercase=False, binary=True))  # POS features
  def extractwordsentif(self):
    ...
    preprocessor = make_pipeline(
      ...
      make_union(
        ...
        CMUArkTweetPOSTagger(),  # POS features
        ...
        make_pipeline(LowercaseTransformer(), CMUArkTweetBrownClusters()),  # brown clusters
        make_pipeline(
          Negater(),
          make_union(
...
class Tokentolist(BaseEstimator, TransformerMixin):
